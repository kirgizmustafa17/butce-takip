...
// const KARAT_22_RATIO = 22 / 24; // No longer needed as we use direct API value
...
export async function fetchMultiplePrices(currencyCodes) {
  try {
    ...
    for (const [key, config] of Object.entries(INVESTMENT_TYPES)) {
      ...
      // Use "Buying" price as requested
      // Data format: data["gram-altin"].Buying -> "3.000,50"
      const item = data[apiCode];
      if (item) {
        price = parseTurkishNumber(item.Buying);
      }
